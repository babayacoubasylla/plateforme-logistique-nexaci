// frontend/src/components/Gerant/HistoryPage.tsx
import React, { useState, useEffect } from 'react';
import { Calendar, Package, Download, Search, Filter, Clock, User } from 'lucide-react';
...
const GerantHistoryPage: React.FC<Props> = ({ onBack, agenceId }) => {
  const [colis, setColis] = useState<Colis[]>([]);
  const [loading, setLoading] = useState(false);
  const [livreurs, setLivreurs] = useState<Array<{ _id: string; nom: string; prenom: string }>>([]);
  ...
  useEffect(() => {
    fetchHistory();
    fetchLivreurs();
  }, [statutFilter, livreurFilter, dateDebut, dateFin]);
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/colis/history/agence/${agenceId}`, { params });
      const data = res.data?.data?.colis || res.data?.colis || [];
      setColis(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error('❌ Erreur fetch historique agence:', error);
      setColis([]);
    } finally {
      setLoading(false);
    }
  ...
  const fetchLivreurs = async () => {
    try {
      // Récupérer les livreurs qui ont travaillé pour cette agence
      const uniqueLivreurs = new Map();
      colis.forEach((c) => {
        if (c.livreur) {
          uniqueLivreurs.set(c.livreur._id || '', {
            _id: c.livreur._id || '',
            nom: c.livreur.nom || '',
            prenom: c.livreur.prenom || ''
          });
        }
      });
      setLivreurs(Array.from(uniqueLivreurs.values()));
    } catch (error) {
  ...
  const downloadReceipt = async (reference: string) => {
    try {
      const res = await api.get(`/api/colis/receipt/ref/${reference}`, { responseType: 'blob' });
  ...
  const exportToCSV = () => {
    ...
    let csv = 'Référence,Statut,Expéditeur,Destinataire,Livreur,Montant,Date\n';
    filteredColis.forEach((c) => {
      ...
      csv += `${c.reference},${c.statut},${exp},${dest},${livreur},${c.tarif.total},${new Date(c.updatedAt).toLocaleDateString()}\n`;
    });
    ...
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `historique-agence-${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    link.remove();
  };
  ...
  const getStatutBadgeClass = (statut: string) => {
    switch (statut) {
      ...
      case 'echec_livraison': return 'bg-red-100 text-red-800';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <div className="flex justify-between items-center">
            <h1 className="text-3xl font-bold text-gray-800">Historique de l'Agence</h1>
            <button
              onClick={exportToCSV}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-lg shadow-sm p-4">
            ...
            <div className="text-2xl font-bold text-gray-800">{filteredColis.length}</div>
          </div>
          <div className="bg-white rounded-lg shadow-sm p-4">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredColis.map((c) => (
                      <tr key={c._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4">
                          <button
                            onClick={() => downloadReceipt(c.reference)}
...
export default GerantHistoryPage;
