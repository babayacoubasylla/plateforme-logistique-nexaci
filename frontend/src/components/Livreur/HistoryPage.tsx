// frontend/src/components/Livreur/HistoryPage.tsx
import React, { useState, useEffect } from 'react';
import { Calendar, Package, FileText, Download, Search, Filter, Clock } from 'lucide-react';
...
const HistoryPage: React.FC<Props> = ({ onBack }) => {
  ...
  }, [activeTab, statutFilter, dateDebut, dateFin]);
  ...
  const fetchColisHistory = async () => {
    ...
    try {
      ...
      const res = await api.get('/api/colis/history/my', { params });
  ...
  const fetchMandatsHistory = async () => {
    ...
    try {
      ...
      const res = await api.get('/api/mandats/history/my', { params });
  ...
  const exportToCSV = () => {
    ...
    let csv = '';
    if (activeTab === 'colis') {
      csv = 'Référence,Statut,Expéditeur,Destinataire,Ville,Montant,Date\n';
      (data as Colis[]).forEach((c) => {
        ...
        const ville = c.destinataire?.ville || c.pointRelais?.ville || 'N/A';
        csv += `${c.reference},${c.statut},${exp},${dest},${ville},${c.tarif.total},${new Date(c.updatedAt).toLocaleDateString()}\n`;
      });
    } else {
      csv = 'Référence,Statut,Client,Document,Administration,Montant,Date\n';
      (data as Mandat[]).forEach((m) => {
        const client = m.client ? `${m.client.prenom} ${m.client.nom}` : 'N/A';
        csv += `${m.reference},${m.statut},${client},${m.type_document.nom},${m.administration.nom},${m.tarif.total},${new Date(m.updatedAt).toLocaleDateString()}\n`;
      });
    }
    ...
    const link = document.createElement('a');
    link.href = url;
    link.setAttribute('download', `historique-${activeTab}-${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    link.remove();
  };
